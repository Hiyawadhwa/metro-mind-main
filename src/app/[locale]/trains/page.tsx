...
export default function MetroMindPage() {
  ...
  return trains.length > 0 && (trains[0] as any).date
    ? new Date((trains[0] as any).date)
    : new Date();
}, [trains]);
...
const trainsData = rawTrains.map((train: any) => ({
  id: train.train_id,
  date: train.date,
 fitness_certificates: {
  rolling_stock: {
    ...
    issue_date: "",
    expiry_date: train.fitness_certificates?.["Rolling Stock Valid"] || "",
    status: "valid",
  },
  signalling: {
    department: "Signalling",
    issue_date: "",
    expiry_date: train.fitness_certificates?.["Signalling Valid"] || "",
    status: "valid",
  },
  telecom: {
    department: "Telecom",
    issue_date: "",
    expiry_date: train.fitness_certificates?.["Telecom Valid"] || "",
    status: "valid",
  },
},
  job_cards: train.job_cards || [],
 branding_contracts: train.branding
  ? [{
      brand: train.branding["Advertiser"] || "",
      total_exposure_hours: 0,
      completed_hours: train.branding["Exposure Hours Today"] || 0,
      deadline: "",
      priority: 0,
      audience_profile: train.branding["Audience Profile"] || "",
      preferred_times: train.branding["Preferred Times"] || "",
    }]
  : [],
  current_mileage: {
  bogie: train.mileage?.["Km Travelled"] || 0,
  brake_pad: 0,
  hvac: 0,
},
maintenance_thresholds: {
  ...
  last_deep_cleaning: train.cleaning?.["Last Deep Cleaned Date"] || "",
  cleaning_duration: 0,
  status: "parking",
  current_position: train.stabling?.Position || null,
}));
...
setTrains(trainsData);
  ...
  const exportReport = () => {
    ...
    a.href = url;
    a.download = `metro_fleet_report_${new Date().toISOString().split("T")[0]
      }.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 text-slate-50 flex">
      <Head>
        <title>{t("title")}</title>
      ...
      <div className="flex-1 md:ml-64 transition-all duration-300">
        <nav className="sticky top-0 z-30 bg-slate-900/70 backdrop-blur border-b border-slate-700">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            {/* Adjusted to hide logo on desktop since sidebar has it, or keep it for mobile */}
            <div className="flex items-center gap-3 md:hidden">
              <h1 className="text-xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-sky-400 to-emerald-400 ml-10">
                MetroMind
              </h1>
            </div>
            {/* For desktop, maybe show the Page Title or Breadcrumb instead of Logo? */}
                      ...
                      {selectedTrain.fitness_certificates.telecom && (
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`w-2 h-2 rounded-full ${selectedTrain.fitness_certificates.telecom
                                .status === "valid"
                                ? "bg-emerald-400"
                                : "bg-rose-400"
                                }`}
                        ...
                        {selectedTrain.job_cards.map((job) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div
                                className={`px-2 py-1 rounded text-xs font-semibold ${job.criticality === "critical"
