...
const getStepsConfig = (
  ...
  <input
    ...
    className="w-full pl-4 pr-4 py-4 rounded-xl border border-slate-600/50 text-slate-50"
    style={{ backgroundColor: "rgba(30,41,59,.6)" }}
  />
</div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          className="w-full p-4 text-center text-2xl font-mono rounded-xl border border-slate-600/50 text-slate-50 placeholder:text-slate-400 tracking-widest outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-400/20"
          style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
          maxLength={6}
        />
      </div>
      ...
      <button
        ...
        className={`w-full py-4 text-lg font-semibold text-slate-50 rounded-xl transition-all duration-300 ${
...
const [formData, setFormData] = useState({
  ...
  if (!formData.employeeId || !formData.password) return;
  ...
  auth,
  employee.emailadd || employee.email,
  formData.password
);

setEmployeeName(employee.fullName || employee.name || "");
setEmployeeRole(employee.role);

const employeeEmail = employee.emailadd || employee.email;

const otpResponse = await fetch(`${API_BASE}/api/send-otp`, {
  method: "POST",
  headers: {
...
if (!otpResponse.ok || !otpData.success) {
  throw new Error(otpData.detail || otpData.error || "Failed to send OTP");
}
...
setCurrentStep(1);
  ...
  try {
    const response = await fetch(
      `${API_BASE}/api/verify-otp?employeeId=${encodeURIComponent(
        formData.employeeId
      )}&otp=${encodeURIComponent(formData.otp)}`
    );
