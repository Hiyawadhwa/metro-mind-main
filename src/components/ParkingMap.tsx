...
export default function AdvancedDepotMap({
  ...
  const DEFAULT_IBL_BAYS = ['IBL01', 'IBL02', 'IBL03', 'IBL04', 'IBL05'];
const DEFAULT_PARKING_TRACKS = Array.from({ length: 12 }, (_, i) => ({
  id: `PT${(i + 1).toString().padStart(2, '0')}`,
  capacity: 2,
}));
  ...
  const iblBays = depotLayout?.ibl_bays || DEFAULT_IBL_BAYS;
const parkingTracks = depotLayout?.parking_tracks || DEFAULT_PARKING_TRACKS;
    ...
    setTrainStates((prev) => {
      ...
      if (
        ...
        newKeys.every((key) => {
          const a = prev[key];
          const b = merged[key];
          return a && b && a.x === b.x && a.y === b.y && a.moving === b.moving;
        })
      ) {
        return prev;
      }
  ...
  setTrainStates((prev) => {
  const merged = { ...initialStates } as Record<
    string,
