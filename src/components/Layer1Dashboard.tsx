...
export default function Layer1Dashboard() {
  ...
  const [unifiedData, setUnifiedData] = useState<{ trains: { id: string; current_position?: string | null }[] } | null>(null);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      // Fetch real current train positions so the depot simulation
      // starts trains from their actual bays instead of all bunching at MAIN_LINE.
      try {
        const unifiedRes = await fetch('/api/nightly/unified-data');
        if (unifiedRes.ok) {
          const unifiedJson = await unifiedRes.json();
          setUnifiedData(unifiedJson);
        }
      } catch (unifiedErr) {
        console.warn('Failed to load unified data for simulation positions:', unifiedErr);
      }
    } catch (e: unknown) {
  ...
  unifiedData={unifiedData || undefined}
