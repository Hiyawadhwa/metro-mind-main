...
def run_layer2_service(
    ...
    service_date: date = None,
    use_layer1_output: bool = True,  # New parameter to use actual Layer 1 output
    readiness_weight: int = 1000,
    shunting_penalty_weight: int = 5000,
    position_bonus_weight: int = 800,
    max_solver_time: int = 120,
) -> Dict[str, Any]:
    """
    Layer 2 optimization: Slot-based train scheduling (1-10 slots)
    Focus on readiness score and minimizing shunting operations
    ...
    readiness_weight / shunting_penalty_weight / position_bonus_weight /
    max_solver_time correspond directly to the fields on
    app.models.OptimizationParams (readiness_weight, shunting_penalty_weight,
    position_bonus_weight, max_solver_time) so callers like
    POST /schedule/advanced can tune the objective function without touching
    this file. Defaults match the values every other caller previously got
    when these were hardcoded, so existing behavior is unchanged unless a
    caller explicitly passes different values.
    """
    try:
        # Use Layer 1 output if requested and available
        if use_layer1_output:
        ...
        # Optimization weights (configurable via run_layer2_service() params;
        # see docstring above -- these mirror OptimizationParams fields)
        READINESS_WEIGHT = readiness_weight          # High weight for train readiness
        SHUNTING_PENALTY = shunting_penalty_weight   # VERY HIGH penalty for shunting operations
        PRIORITY_SLOT_BONUS = 2000       # Bonus for high-readiness trains in priority slots (1-3)
        POSITION_BONUS_WEIGHT = position_bonus_weight  # Bonus for trains in better positions (front of bay)
        BRANDING_URGENCY_WEIGHT = 50     # Low weight; readiness + shunting dominate
