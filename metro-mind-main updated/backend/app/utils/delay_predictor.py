...
Predicts per-station arrival delays for a train's daily rotation.

`DelayPredictor.predict_on_schedule()` walks each train's scheduled
station events and, for every event, decides whether a delay occurs and
how large it is:

- A trained RandomForestClassifier (`backend/models/delay_classifier.pkl`)
  estimates the probability of a delay at that station/time/condition
  combination. `_ml_should_predict_delay()` gates the decision on that
  probability, with the original rule-based checks (critical job cards,
  severe weather) kept as a safety net so those cases are never missed
  even if the model's probability comes out low.
- A trained GradientBoostingRegressor (`backend/models/gb_regressor.pkl`)
  estimates the delay amount in minutes. `_blend_delay_amount()` blends
  that estimate with the rule-based estimate from `_get_delay_amount()`,
  which encodes domain knowledge (job card severity, weather, peak hours,
  station-specific effects, equipment fatigue) the regressor may not have
  learned well from a limited synthetic training set.

Both models, plus the categorical encoders and feature column order they
were trained with, are produced by `train_delay_models.py` (trained on
data from `generate_synthetic_delay_data.py`) and loaded lazily via
`_safe_load()` -- if a model file is missing, prediction falls back to
the rule-based logic alone rather than raising.

Delays cascade within a train's rotation (`prev_cum_delay` carries
forward, decayed) and, above `MAJOR_DELAY_THRESHOLD`, get recorded via
`_track_major_delay()` for downstream cascade-impact reporting.
"""
...
class DelayPredictor:
    ...
    def _ml_should_predict_delay(self,
                                pred_prob: float,
                                critical_delay_jobcards: int,
                                high_delay_jobcards: int,
                                weather_type: str,
                                crowd_level: int,
                                hour: int) -> bool:
        """Gate the delay decision on the trained classifier's probability.

        The classifier is trained on realistic delay rates (~15% of events
        have any delay at all), so `pred_prob >= 0.5` is the primary signal.
        The original rule-based checks are kept as an OR'd-in safety net for
        the situations that matter most operationally -- a critical job card
        or severe weather -- so those never get silently waved through just
        because the model's probability happened to come out low for a
        combination it saw few examples of during training.

        Falls back to the rule-based method entirely if no classifier is
        loaded.
        """
        if self.classifier is None:
            return self._should_predict_delay(
                critical_delay_jobcards, high_delay_jobcards,
                weather_type, crowd_level, hour
            )

        ml_says_delay = pred_prob >= 0.5
        safety_net = (
            critical_delay_jobcards > 0
            or weather_type in ("storm", "heavy_rain")
        )
        return ml_says_delay or safety_net

    def _blend_delay_amount(self,
                           ml_predicted_minutes: float,
                           rule_based_minutes: float,
                           pred_prob: float) -> float:
        """Blend the regressor's predicted delay with the rule-based estimate.

        Weight the ML prediction more heavily when the classifier is more
        confident a delay is actually happening; otherwise lean on the
        rule-based estimate, which already encodes domain knowledge (job
        card severity, weather, peak hours, station-specific effects) that
        the regressor may not have learned well from a limited synthetic
        training set. The weight is clamped to [0.3, 0.7] so neither source
        ever fully dominates the other.
        """
        if ml_predicted_minutes is None:
            return rule_based_minutes

        ml_weight = min(max(pred_prob, 0.3), 0.7)
        blended = (ml_weight * ml_predicted_minutes) + ((1 - ml_weight) * rule_based_minutes)
        ...
        for train in baseline_rotation.get("train_schedules", []):
            ...
            for ev in train.get("station_events", []):
                ...
                try:
                    ...
                    # Get ML probability from the trained classifier
                    pred_prob = 0.0
                    if self.classifier is not None:
                        try:
                            pred_prob = float(self.classifier.predict_proba(features_df)[:, 1][0])
                    ...
                    # Get ML delay-amount estimate from the trained regressor
                    ml_predicted_minutes = None
                    if self.regressor is not None:
                        try:
                            ml_predicted_minutes = float(max(0.0, self.regressor.predict(features_df)[0]))
                        except Exception:
                            ml_predicted_minutes = None

                    # Decide if we should predict a delay -- classifier-gated,
                    # with the rule-based checks as an OR'd-in safety net
                    should_predict_delay = self._ml_should_predict_delay(
                        pred_prob, critical_delay_jobcards, high_delay_jobcards,
                        weather_type, crowd_level, hour
                    )
                    ...
                    # Blend in the regressor's estimate
                    if ml_predicted_minutes is not None:
                        predicted_minutes = self._blend_delay_amount(
                            ml_predicted_minutes, predicted_minutes, pred_prob
                        )
