...
#             data = json.load(f)
#         trains = [train.get("train_id") for train in data.get("data", []) if train.get("train_id")]
...
#         train_update = {"train_id": update.train_id}
...
#                 if train.get("train_id") == update.train_id:
#                     current_train = train
#                     break
...
#             current_dir, "..", "storage", "unified.json"
#         )
...
def add_branding(append_req: BrandingAppendModel):
    ...
    # Save historical snapshot (same audit-trail pattern as /update/train)
    today = datetime.now().strftime("%Y-%m-%d")
    snapshot = {
        "date": today,
        "source": "branding",
        "train_id": append_req.train_id,
        "data": new_branding_contract,
    }
    try:
        with open(HISTORICAL_JSON_PATH, "r") as f:
            historical = json.load(f)
...
def update_parking_assignment(train_id: str, assignment: ParkingAssignmentModel):
    ...
    try:
        ...
        input_id_norm = normalize_id(train_id)
        found = False
        for i, existing in enumerate(assignments):
            if normalize_id(existing["train_id"]) == input_id_norm and not existing.get("departure_time"):
                for other in assignments:
                    if (normalize_id(other["train_id"]) != input_id_norm and 
                        other["bay"] == assignment.bay and 
                        other["position"] == assignment.position and 
                        not other.get("departure_time")):
                        raise HTTPException(status_code=400, detail=f"Position {assignment.bay}-{assignment.position} is already occupied")
...
def delete_parking_assignment(train_id: str):
    ...
    try:
        ...
        found = False
        for i, assignment in enumerate(assignments):
            if normalize_id(assignment["train_id"]) == input_id_norm and not assignment.get("departure_time"):
                assignments[i]["departure_time"] = datetime.now().isoformat()
                found = True
                break
...
async def get_unified_data():
    try:
        # Use the shared config path (respects the UNIFIED_JSON_PATH env var
        # override) instead of recomputing it from this file's location --
        # the old local recomputation always pointed at backend/storage/
        # regardless of what UNIFIED_JSON_PATH was configured to.
        with open(UNIFIED_JSON_PATH, 'r') as file:
            data = json.load(file)
        return data
    except FileNotFoundError:
