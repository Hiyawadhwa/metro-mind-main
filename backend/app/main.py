...
try:
    ...
    # ast.literal_eval raises these for a malformed CORS_ORIGINS env value
    # (e.g. not valid Python literal syntax). Narrowed from a bare `except:`
    # so unrelated errors (KeyboardInterrupt, SystemExit, etc.) aren't
    # silently swallowed here.
    logging.getLogger(__name__).warning(
        f"Invalid CORS_ORIGINS env value ({CORS_ORIGINS!r}), falling back to defaults: {e}"
    )
...
@app.get("/")
async def root():
    return {"message": "Welcome to MetroMind Backend. Metro-Mind API is running."}
...
async def generate_data():
    ...
    try:
        unified_path = os.path.join(STORAGE_DIR, "unified.json")
        REQUIRED_KEYS = {
            "trains", "cleaning_slots", "depot_layout", "date",
            "required_trains", "standby_trains", "cleaning_crew_available",
        }

        # Only preserve the existing file if it's actually in the canonical
        # schema ScheduleOptimizer expects. Otherwise it silently returns
        # a raw-schema file that later 500s in /optimize with KeyError: 'trains'.
        if os.path.exists(unified_path):
            with open(unified_path, "r", encoding="utf-8") as f:
                existing = json.load(f)
            if REQUIRED_KEYS.issubset(existing.keys()):
                logger.info("Existing unified.json is in canonical schema; preserving it")
                return {"message": "Existing unified.json preserved; no overwrite", "data": existing}
            logger.warning("Existing unified.json is missing required keys %s — regenerating",
                            REQUIRED_KEYS - existing.keys())

        logger.info("Generating synthetic data...")
        generator = DataGenerator()
        generated = generator.generate_all_data()

        os.makedirs(STORAGE_DIR, exist_ok=True)
        with open(unified_path, "w", encoding="utf-8") as f:
            json.dump(generated, f, indent=2)

        logger.info("Synthetic unified.json generated successfully")
        return {"message": "Synthetic data generated", "data": generated}
...
def schedule_advanced(payload: ScheduleRequest, params: OptimizationParams = None):
    ...
    try:
        ...
        result = run_layer2_service(
            ...
            service_day=payload.service_day or "weekday",
            service_date=target_date,
            readiness_weight=params.readiness_weight,
            shunting_penalty_weight=params.shunting_penalty_weight,
            position_bonus_weight=params.position_bonus_weight,
            max_solver_time=params.max_solver_time,
        )
        
        # Add optimization parameters to response
        result["optimization_params"] = params.dict()
        result["input_validation"] = validation
        result["processing_time"] = datetime.now().isoformat()
        result["note"] = "Ad-related parameters are deprecated and ignored."
        result["optimization_focus"] = "Readiness scores and minimal shunting operations"
...
async def send_otp(request: OTPRequest):
    try:
        ...
        # Determine recipient email
        # Hardcoded master authorized email as per user request
        recipient_email = request.email  # or hiya.wadhwa.mail@gmail.com directly
